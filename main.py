...
if __name__ == "__main__":
    ...
    # Taking inputs for stake and odds from the sportsbook
    # Taking inputs for stake and odds from the sportsbook
    sportsbook_stake = float(input("Enter the stake at the sportsbook: "))
    sportsbook_odds = float(input("Enter the odds taken at the sportsbook: "))
    ...
    print(f"Liability on lay required to achieve the profit: {abs(overall_stake - sportsbook_stake):.2f}")
    print(f"The lay stake should be: {lay_potential_winnings:.2f}")
    print(f"Overall Stake: {overall_stake:.4f}")
    print(f"Return if Sportsbook wins: {sportsbook_win_return - overall_stake :.4f}")
    print(f"Margin if Sportsbook wins: {margin_if_sportsbook_wins:.4f}%")
    print(f"Return if Lay wins: {lay_win_return_with_commission - sportsbook_stake :.4f}")
    print(f"Margin if Lay wins: {margin_if_lay_wins:.4f}%")